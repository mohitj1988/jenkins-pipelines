    ...
    stages{
        stage('UI tests Upgrade Matrix') {
            parallel {
                ...
                stage('Run CLI tests @cli & @docker-configuration'){
                    steps {
                        script {
                            runUITestsJob(GIT_BRANCH, GIT_COMMIT_HASH, DOCKER_VERSION, CLIENT_VERSION, '@cli|@docker-configuration', MYSQL_IMAGE, POSTGRES_IMAGE, MONGO_IMAGE, PROXYSQL_IMAGE, PMM_QA_GIT_BRANCH, '');
                        }
                    }
                }
            }
